...
class StudentEval(Task):
    ...
    def process_results(self, generations, references):
        ...
        for ref in references:
            problem_id_to_ref[ref["problem"]] = ref
        worklist = []
        for gens in generations:
            for generation in gens:
                if not generation.startswith("def "):
                    raise ValueError("Must contain prompt")
                problem_name = generation.split("def ")[-1].split("(")[0].strip()
                reference = problem_id_to_ref[problem_name]
                assert reference["problem"] in generation.split("\n")[0]
                item = {
        ...
        # save solutions
        results_df[["problem","program","success"]].to_csv("/tmp/bigcode_results.csv")
